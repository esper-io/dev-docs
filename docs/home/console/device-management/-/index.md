...
:::note In order to capture a bug report from a device, the device cannot be in "Kiosk" mode—which prevents the notification bar from being displayed—and the notification bar must be enabled. If the target device is in "Kiosk" mode or the notification bar has not been enabled, you will be unable to capture a bug report :::
